...
class VideoStream(ABC):
    def __init__(self, video_stream_params={}, **kwargs):
        ...
        if self.save_video or self.show_algo:
            self.recording_thread = Thread(
                target=self.start_recording, args=())
    ...
    def iterate_video(self):
        ...
        if self.playback:
            ...
            file_name_params = [self.name, self.algorithm,
                                'Playback', playback_file[:-4], '.avi']
            self.video_file = self.storage_location + \
                '_'.join(file_name_params)
        else:
            self.frame_name = '_'.join(
                [self.name, self.algorithm, 'Stream'])
    ...
    def move_files(self):
        ...
        for file in os.listdir(sl):
            os.replace(sl + file, sl + 'sync/' +
                       file) if '.txt' in file or '.avi' in file else 0
...
if __name__ == '__main__':
    ...
    video_stream = VideoStream(
        video_stream_params=video_stream_params)
