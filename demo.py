...
class Demo(VideoStream):
        ...
        while True:
            try:
                if self.thread_manager['run_algo']:
                    ...
                    gray = cv2.cvtColor(
                        self.algorithm_frame, cv2.COLOR_BGR2GRAY)
                    gray = cv2.GaussianBlur(
                        gray, (self.blur, self.blur), 0)
                    frame_delta = cv2.absdiff(self.img_old, gray)
                    self.img_old = gray
                    thresh = cv2.threshold(
                        frame_delta, self.move_thresh, 255, cv2.THRESH_BINARY)[1]
    ...
    def report_motion(self):
        ...
        for idx, region in enumerate(self.roi_names):
            ...
            if self.si_dict[region]['motion_flag'] or self.si_dict[region]['message_type'] == 'Event':
    ...
    def reset_si_dict(self):
        dict_keys = {'motion_locs': [], 'object_locs': [], 'object_type': [], 'object_confidence': [],
                     'motion_flag': None, 'motion_max_delta': [], 'motion_max_delta_idx': [],
                     'message_type': []}
    ...
    def set_si_dict(self):
        mm_names, motion_regions, motion_roi, mpa, path = set_region_roi(self.name, self.frame_width,
                                                                         self.frame_height)
        ...
        xy_list = [(x, y) for x in range(self.frame_width)
    ...
    def write_data_headers(self):
        with open(self.si_file, 'a') as file:
            headers = ['Time', 'Region', 'Object',
                       'Confidence', 'Location', 'MessageType']
            ...
            for region in self.roi_names:
                ...
                if self.si_dict[region]['object_type'] != []:
                    for idx, obj in enumerate(self.si_dict[region]['object_type']):
                        ...
                        out_data.append(
                            self.si_dict[region]['message_type'][idx])
...
video_stream = Demo(
    video_stream_params=video_stream_params, **algo_params)
