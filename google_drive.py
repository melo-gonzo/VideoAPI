    ...
    if local_files:
        while local_files:
            ...
            file = drive.CreateFile(
                {"mimeType": "image/jpeg", "parents": [{"kind": "drive#fileLink", "id": esp_folder_id}]})
...
def get_drive_files(esp_folder_id):
    fileList = drive.ListFile(
        {'q': f"\'{esp_folder_id}\' in parents and trashed=false"}).GetList()

    creation_dict = {}
    for file in fileList:
        creation_date = file['createdDate']
        time_stamp = datetime.timestamp(datetime.strptime(
...
def clean_up_drive_files(creation_dict):
    creation_dict = sorted(creation_dict.items(),
